...
import simpleCoffeeList from './simple-coffee-listing.json';
export default function App() {
  const [list, setList] = useState(simpleCoffeeList);
  const [activeBtn, setActiveBtn] = useState(1);
  return (
    <div className="App">
      <Header />
      <Filter setList={setList} activeBtn={activeBtn} setActiveBtn={setActiveBtn} />
      <CoffeeList list={list} setList={setList} setActiveBtn={setActiveBtn} />
    </div>
  );
}
...
function Filter({ setList, setActiveBtn, activeBtn }) {
...
function Button({ text, onClickHandler, num, activeBtn, setActiveBtn }) {
  return <button className={activeBtn == num ? "active" : ''} onClick={() => {
    onClickHandler();
    setActiveBtn(btn => Number(num));
  }}>{text}</button>
}
  ...
  return (
    <>
      <div className='card-container'>
        {list.map(coffee => <Card url={coffee.image} name={coffee.name} price={coffee.price} rating={coffee.rating} votes={coffee.votes} popular={coffee.popular} soldOut={!coffee.available} key={coffee.id} />)}
      </div>
      <div className='sort-the-list'>
        <label for="sort-by">Sort By</label>
        <select id='sort-by' onChange={(e) => { manageSorting(e.target.value) }}>
...
function Card({ url, name, price, rating, votes, soldOut, popular }) {
  return (<div className="card">
    ...
    <div className='flex space-between'>
      {rating ? <p className='rating'><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      </svg>{rating}<span className='votes'>({votes} votes)</span></p> : <p className='rating'><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <span className='soldout'> {soldOut && "Sold out"}</span>
    </div >
  </div >
  )
